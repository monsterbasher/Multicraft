...
namespace Aurora
{
	...
	void Camera::lookAt(const Vector3 &eye, const Vector3 &target, const Vector3 &up)
	{
		...
		m_accumPitchDegrees = Math::Math::radiansToDegrees(-asinf(m_viewMatrix[1][2]));
	}
	...
	void Camera::perspective(float fovx, float aspect, float znear, float zfar)
	{
		...
		float e = 1.0f / tanf(Math::Math::degreesToRadians(fovx) / 2.0f);
	...
	void Camera::updatePosition(const Vector3 &direction, float elapsedTimeSec)
	{
		...
		if (m_currentVelocity.magnitudeSq() != 0.0f)
		{
			...
			if (direction.x == 0.0f && Math::Math::closeEnough(m_currentVelocity.x, 0.0f))
				displacement.x = 0.0f;

			if (direction.y == 0.0f && Math::Math::closeEnough(m_currentVelocity.y, 0.0f))
				displacement.y = 0.0f;

			if (direction.z == 0.0f && Math::Math::closeEnough(m_currentVelocity.z, 0.0f))
				displacement.z = 0.0f;
